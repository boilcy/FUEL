...
namespace fast_planner
{
  ...
  void MapROS::init()
  {
    ...
    pose_sub_.reset(new message_filters::Subscriber<nav_msgs::Odometry>(node_, "/map_ros/pose", 25));
  ...
  {
    // transform img (in frame/camera_1_optical) to frame /Drone_1/odom_local_ned

    /* tf::StampedTransform transform;
    try
    {
      tf_.lookupTransform(pose->child_frame_id, img->header.frame_id, img->header.stamp, transform);
    }
    ...
    tf::Pose tfed_pose = transform * tf_pose;

    tf::Vector3 tf_t = tfed_pose.getOrigin();
    tf::Quaternion tf_q = tfed_pose.getRotation();

    camera_pos_ = Eigen::Vector3d(tf_t.x(), tf_t.y(), tf_t.z());
    camera_q_ = Eigen::Quaterniond(tf_q.w(), tf_q.x(), tf_q.y(), tf_q.z()); */

    /* root@fuelhost:~/catkin_ws# rosrun tf tf_echo /Drone_1/odom_local_ned /camera_1_optical
    At time 1719308543.443
    - Translation: [0.500, -0.060, 0.100]
    - Rotation: in Quaternion [0.500, 0.500, 0.500, 0.500]
                in RPY (radian) [1.571, -0.000, 1.571]
                in RPY (degree) [90.000, -0.000, 90.000] */
    
    /* Eigen::Vector3d T2(0.500, -0.060, 0.100);
    Eigen::Quaterniond q2(0.500, 0.500, 0.500, 0.500);
    q2.normalize();

    Eigen::Matrix4d transformationMatrix = Eigen::Matrix4d::Identity();
    transformationMatrix.block<3, 3>(0, 0) = q2.toRotationMatrix();
    transformationMatrix.block<3, 1>(0, 3) = T2;
  ...
  void MapROS::proessDepthImage()
  {
    ...
    for (int v = depth_filter_margin_; v < rows - depth_filter_margin_; v += skip_pixel_)
    {
      ...
      for (int u = depth_filter_margin_; u < cols - depth_filter_margin_; u += skip_pixel_)
      {
        ...
        /* // using airsim gt odom
        pt_cur(0) = (u - cx_) * depth / fx_; 
        pt_cur(1) = depth;
        pt_cur(2) = (cy_ - v) * depth / fy_; */
        pt_cur(0) = depth;
